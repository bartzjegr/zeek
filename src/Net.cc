...
void net_init(const string& interface, const string& readfile,
	...
	if ( ! readfile.empty() )
		...
		iosource::PktSrc* ps = iosource_mgr->OpenPktSrc(readfile, false);
		assert(ps);

		if ( ! ps->IsOpen() )
			reporter->FatalError("problem with trace file %s (%s)",
				readfile.c_str(), ps->ErrorMsg());
		}
...
void net_get_final_stats()
	...
	if ( ps && ps->IsLive() )
		...
		reporter->Info("%" PRIu64 " packets received on interface %s, %" PRIu64 " (%.2f%%) dropped",
			       s.received, ps->Path().c_str(), s.dropped, ((double)s.dropped / ((double)s.received + (double)s.dropped)) * 100.0);
		}
	}
